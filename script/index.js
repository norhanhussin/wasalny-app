...
const loadStatsAndTrips = () => {
    ...
    // 2. تحديث الإحصائيات بناءً على الرحلات المقبولة فقط
    document.getElementById('totalTrips').innerText = approvedTrips.length;
    document.getElementById('totalWeight').innerText = approvedTrips.reduce((a, b) => a + Number(b.weight || 0), 0);

    const latestContainer = document.getElementById('latestTripsRow');

    if (approvedTrips.length > 0) {
        // عرض آخر 3 رحلات تم قبولها (الأحدث يظهر أولاً)
        latestContainer.innerHTML = approvedTrips.slice(-3).reverse().map((t, index) => `
                <div class="col-md-4" data-aos="fade-up" data-aos-delay="${index * 100}">
                    <div class="home-trip-card">
                        ...
                        <div class="fw-black fs-5 mb-2">${t.from} <i class="fas fa-arrow-left mx-2 text-muted small"></i> ${t.to}</div>
